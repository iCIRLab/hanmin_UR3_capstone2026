...
아래 UR3 영상은 계획 없이 목표 관절각을 직접 전송하는 `ur3_pose_control` 구성에서 촬영했습니다.
...
자체 제작 그리퍼를 장착해 물체를 파지한 시험입니다. 그리퍼 제어는 아직 이 스택에 통합되지 않았습니다.
...
- **보정**: 실제 UR3에서 추출한 `ur3_calibration.yaml`이 URDF와 MJCF 양쪽에 반영됩니다
...
```text
src/
├── capstone_bringup/            # 통합 런치 (MuJoCo·실제 UR3)
│   └── launch/
...
│       └── ur3_pose_control.py       # 검증된 키보드 Home/S 제어 (보존)
...
루트의 `ur3_calibration.yaml`은 실제 UR3에서 추출한 값이며
